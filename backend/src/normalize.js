...
export function loadRawHostaway() {
  // Resolve relative to this module file so it works no matter the CWD (tests, dev, prod)
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);
  const file = path.join(__dirname, '..', 'mock', 'hostawayReviews.json');
  if (!fs.existsSync(file)) {
    throw new Error(`Mock reviews file not found at ${file}`);
  }
  const raw = JSON.parse(fs.readFileSync(file, 'utf-8'));
  return raw.result || [];
}

// Normalize reviews into structure grouped by listing
// Output shape:
// {
//   listings: {
//     [listingName]: {
...
//        aggregates: { avgOverall, totalReviews, categories: { cleanliness: { avg, count }, ... } }
//     }
//   },
//   meta: { generatedAt, listingCount, reviewCount }
// }
